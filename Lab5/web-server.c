...
int main() {
    ...
    read_html("index.html", html_content, sizeof(html_content));
    ...
    while (1) {
        ...
        sscanf(output_buffer, "%s %s %s %s", elevator_id, count, date, time);
        snprintf(json_content, sizeof(json_content), "[{\"elevator_id\":%s,\"count\":%s,\"time\":\"%s-%s\"}]", elevator_id, count, date, time);
