...
int main() {
    ...
    read_html("./static/index.html", html_content, sizeof(html_content));
    ...
    while (1) {
        my_db.query(&my_db, "SELECT * FROM people_counting ORDER BY time DESC LIMIT 1");
        my_db.store_result(&my_db);
        my_db.output(&my_db, output_buffer);
        sscanf(output_buffer, "%s %s %s %s %s", elevator_id, inside_count, outside_count, date, time);
        snprintf(json_content, sizeof(json_content), "[{\"elevator_id\":%s,\"inside_count\":%s,\"outside_count\":%s,\"time\":\"%s-%s\"}]", elevator_id, inside_count, outside_count, date, time);
